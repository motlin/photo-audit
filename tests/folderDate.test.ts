...
describe("folderDateFor", () => {
	it("finds the date of the nearest dated ancestor folder", () => {
		expect(
			folderDateFor("/photos/2019/2019-01-01 Party/IMG_1.jpg", "/photos"),
		).toEqual({ year: 2019, month: 1, day: 1, time: null });
	});

	it("returns null when no ancestor folder is dated", () => {
		expect(
			folderDateFor("/photos/Image Capture/IMG_4309.jpg", "/photos"),
		).toBeNull();
	});
